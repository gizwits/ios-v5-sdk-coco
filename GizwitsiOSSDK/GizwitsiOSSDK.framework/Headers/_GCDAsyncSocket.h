...
@protocol _GCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const _GCDAsyncSocketException;
extern NSString *const _GCDAsyncSocketErrorDomain;

extern NSString *const _GCDAsyncSocketQueueName;
extern NSString *const _GCDAsyncSocketThreadName;

extern NSString *const _GCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const _GCDAsyncSocketUseCFStreamForTLS;
#endif
...
extern NSString *const _GCDAsyncSocketSSLPeerID;
extern NSString *const _GCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const _GCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const _GCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const _GCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const _GCDAsyncSocketSSLCipherSuites;
extern NSString *const _GCDAsyncSocketSSLALPN;
#if !TARGET_OS_IPHONE
extern NSString *const _GCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define _GCDAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(_GCDAsyncSocketErrorDomain, _GCDAsyncSocketError) {
...
@interface _GCDAsyncSocket : NSObject
...
- (instancetype)initWithDelegate:(nullable id<_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<_GCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<_GCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<_GCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<_GCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<_GCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
 ...
 *
 * - _GCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
...
@protocol _GCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(_GCDAsyncSocket *)sock;
...
**/
- (void)socket:(_GCDAsyncSocket *)sock didAcceptNewSocket:(_GCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(_GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(_GCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(_GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(_GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(_GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(_GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(_GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(_GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(_GCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(_GCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socketDidSecure:(_GCDAsyncSocket *)sock;
...
**/
- (void)socket:(_GCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
